...
mem = s:taboption(this_tab, ListValue, "storage", translate("信息存储区"), translate("信息存储在一个特定的位置(例如, 在 SIM 卡或调制解调器内存), 但根据设备的类型, 其他区域也可能是可用的."))
mem.default = "SM"
...
local ta = s:taboption(this_tab, TextValue, "user_phonebook", translate("用户电话簿"), translate("每一行必须有以下格式. '联系人;号码'. 保存到文件'/etc/config/phonebook.user'."))
ta.rows = 7
ta.rmempty = false
...
dev3 = s:taboption(this_taba, Value, "ussdport", translate("USSD 发送端口"))
...
local u = s:taboption(this_taba, Flag, "ussd", translate("以纯文本发送 USSD 代码"), translate("以纯文本发送 USSD 代码. 命令没有被编码到 PDU 中."))
u.rmempty = false

local p = s:taboption(this_taba, Flag, "pdu", translate("接收没有 PDU 解码的信息"), translate("接收并显示消息, 而不将其解码为 PDU."))
p.rmempty = false

local tb = s:taboption(this_taba, TextValue, "user_ussd", translate("用户 USSD 代码"), translate("每一行必须有以下格式. '名称;代码'. 保存到文件'/etc/config/ussd.user'."))
tb.rows = 7
tb.rmempty = true
...
dev4 = s:taboption(this_tabc, Value, "atport", translate("AT 命令端口"))
...
local tat = s:taboption(this_tabc, TextValue, "user_at", translate("用户 AT 命令"), translate("每一行必须有以下格式. '名称;命令'. 保存到文件'/etc/config/atcmds.user'."))
tat.rows = 20
tat.rmempty = true
...
local uw = s:taboption(this_tabb, Flag, "lednotify", translate("通知新消息"), translate("LED 通知有新的信息. 在激活这个功能之前, 请配置并保存短信阅读端口,,检查短信收件箱的时间, 并选择通知 LED."))
uw.rmempty = false
...
local time = s:taboption(this_tabb, Value, "checktime", translate("每几分钟检查一次收件箱"), translate("指定你想在多少分钟内检查你的收件箱."))
time.rmempty = false
time.maxlength = 2
time.default = 5
...
leds = s:taboption(this_tabb, Value, "smsled", translate("通知 LED"), translate("选择通知 LED."))
if try_leds then
local node
local status
...
oled = s:taboption(this_tabb, ListValue, "ledtype", translate("该二极管只专门用于这些通知"), translate("如果路由器只有一个 LED, 或者 LED 是多任务的,就选 'No'."))
oled.default = "D"
oled:value("S", translate("No"))
oled:value("D", translate("Yes"))
oled.rmempty = true

local timeon = s:taboption(this_tabb, Value, "ledtimeon", translate("每几秒打开 LED 灯"), translate("指定 LED 应该亮多长时间."))
timeon.rmempty = false
timeon.maxlength = 3
timeon.default = 1

local timeoff = s:taboption(this_tabb, Value, "ledtimeoff", translate("每几秒关闭 LED 灯"), translate("指定 LED 应该关闭多长时间."))
